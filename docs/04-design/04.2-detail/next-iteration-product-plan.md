...
5. **本机诊断**
   - 产品接入失败时，开发者能用 `ms doctor` 快速检查 server、配置、scope、数据库和接口健康。
...
| P0-2 | Project Memory Workspace：`ms init` / project manifest / 多 source root | `ms init` 只创建 scope identity 和 manifest（**不强制目录索引**） | 完整 `ms project` 命令族 + 多 source root + scanner 改造 + 增量 evidence |
...
| 支撑项 | 范围 |
|--------|------|
| `ms demo/connect/status` | 只用于验证单 appId 的 Working Context 接入闭环；`doctor` 保持最小诊断 |
...
| `.mengshu.json` 解析器 | 中 | 文件格式 + 解析 + lookup 链路均不存在 |
...
> 跨 appId 复用 profile/rules 默认开启（同一用户授权范围内），但用户可在 `ms init` 或 Console 中关闭。task_context/experience 默认不跨 project 复用，需用户显式提升。
...
request.scope.workspaceId/projectId
  -> .mengshu.json in cwd or ancestor
...
| 动作 | appId | 输入来源 | 解析结果 |
|------|-------|----------|----------|
| `ms init` | cli | 当前目录 | 创建 `workspaceId=ws-acme`、`projectId=proj-acme` |
...
1. 同一 `.mengshu.json` 下，不同 `appId` 对同一 `userId/workspaceId/projectId` 的 `profile/rules` 可复用。
...
| Situated action | 工作行为发生在具体目录、文件、工具和会话情境中 | `ms init` 建立 Project Workspace；Evidence 绑定文件、会话、tool result 和 source root | project context 召回可追溯到本地 evidence |
...
| 套件 | 新增 case | 判定方式 | 期次 |
|------|-----------|----------|------|
| `mengshu-v0.1` | 偏好复用（单 appId）、目标更新、资源速查 | 机械化（sourceId 命中 / lookup 命中） | v0.1 |
| `mengshu-v0.1` | 重复说明减少 | **需 LLM judge / 人工标注**（跨会话语义相似度） | **v0.3 行为监控** |
| `mengshu-cross-product` | 产品 A 写入偏好，产品 B 可用；A 的 project task 不泄漏到 B 的另一 project | 机械化（依赖跨 appId 召回） | **v0.2** |
| `mengshu-safety` | 人格标签不自动生成；敏感属性不写入；stale task 不注入；冲突偏好不自动覆盖 | 机械化（黑名单字符串检测 + lifecycle 断言） | v0.1 |
| `mengshu-behavior` | 采纳率（注入 rule 后 agent 是否遵守） | **需 LLM judge + 行为轨迹**，无采集 pipeline | **v0.3 行为监控** |
...
| Project workspace lifecycle | `ms init` / `ms project refresh` | project manifest + source root delta + index/tree jobs |
...
> Project Memory Workspace 是用户工作上下文的本地 project root。用户在某个目录执行 `ms init` 后，mengshu 为该目录建立 project identity、scope、manifest、source roots 和增量更新链路。
...
1. 在项目目录生成轻量 `.mengshu.json` 指针。
2. 在用户本地全局库创建 `~/.mengshu/projects/<project-id>/manifest.json`。
3. 建立默认 source root：当前目录为 `role=project_root`，并应用 include/exclude。
4. 创建初始 indexing job，后续由 `ms project refresh` 或 watch 增量更新。
...
| `ms project refresh` | 本地文件系统变化、目录层级变化 | Document/Chunk delta、source/topic/global tree stale/seal、resource candidate |
...
当前已有 `ms serve/status/health/migrate`（`adapters/openclaw/cli.ts`）和 `list/tables/stats/search/query/scan/cleanup/export/kb/rules`（`index.ts`）。下一迭代补 project workspace 命令、产品化接入命令和数据销毁命令（init/project/doctor/demo/connect/reset 当前均不存在）：

| 命令 | 作用 |
|------|------|
| `ms init` | 在当前目录初始化 Project Memory Workspace |
| `ms project status` | 查看 project scope、source roots、索引新鲜度、candidate backlog 和失败 job |
| `ms project add-root <path>` | 增加一个本地 source root，并声明 role/include/exclude |
| `ms project index` | 首次索引当前 project workspace |
| `ms project refresh` | 基于 manifest diff 和 contentHash 做增量更新 |
| `ms project watch` | 监听目录变化，批量触发 refresh job |
| `ms project commit` | 把当前 session 摘要、决策和资源变化写回 project memory |
| `ms project lookup <query>` | 在当前 project scope 下速查记忆、资源和 evidence |
| `ms project context` | 预览当前 5 slot context |
| `ms doctor` | 分级检查配置、server、DB、embedding、scope、REST、Console、磁盘、job、candidate backlog、迁移状态 |
| `ms demo` | 写入一组用户工作上下文 demo 记忆，并演示不同 `appId` 下的 context/lookup |
| `ms connect openclaw` | 输出 OpenClaw adapter 接入配置、server URL、secret、scope 示例（默认不覆盖已有配置，`--force` 显式覆盖） |
| `ms reset --project <id>` | 销毁指定 project workspace 的本地数据（隐私要求，需二次确认） |

`ms doctor` 至少覆盖 10 项，按 `[ok|warn|fatal]` 分级，输出面向产品开发者：
...
> 基目录说明：当前默认库路径是 `~/.openclaw/memory/lancedb`。v0.x 保持此默认以兼容已发布版本；project manifest 放在 `~/.openclaw/memory/projects/<project-id>/manifest.json`，与库同基目录，不单独引入 `~/.mengshu/`，避免迁移成本。如未来需要产品中性基目录，再通过 `ms migrate` 提供迁移。
...
| 套件 | 数量 | 目的 | 通过门槛 | 期次 |
|------|------|------|----------|------|
| `mengshu-v0.1` | ≥ 30 | 单 appId 的 Agent context、lookup、SlotSnapshot、lookup-only | recall@5 ≥ baseline + 5pt | **v0.1** |
| `mengshu-safety` | ≥ 40 | private/revoked/stale/conflict 不误注入 + 人格标签/敏感属性不写入 | 误注入率 = 0（硬门槛） | **v0.1** |
| `mengshu-cross-product` | ≥ 30 | 同一用户工作上下文在不同 `appId` 下的连续性 | cross-app recall ≥ 80%，无 P1 case 退化 | **v0.2** |
| `mengshu-behavior` | ≥ 20 | 重复解释减少、采纳率 | 需 LLM judge，先定性 | **v0.3 研究分支** |
...
2. `eval/goldens/mengshu-v0.1.jsonl`（≥ 30，**单 appId**）和 `mengshu-safety.jsonl`（≥ 40）。**cross-product 黄金集延后 v0.2**。
...
目标：`ms init` 可创建稳定 project scope identity 和 manifest，**不强制目录索引**。完整 Project Workspace（多 source root + scanner 改造 + 增量 evidence）延后 v0.2。

> v2 评审修正：原 Milestone A2 含 6 个新模块（ms project 命令族 + manifest 解析 + scanner 改造 + 第二 appId adapter），3-4 周。A2-lite 只做 identity 和 scope 入口（1 周内），目录索引延后。

交付：

1. `ms init` 创建 `.mengshu.json`（§schema）和本地 project manifest。
2. manifest 包含 `workspaceId/projectId/defaultVisibility/slotReusePolicy`（sourceRoots 字段保留但 v0.1 可为空）。
3. `ms project status/context/lookup` 最小可用（基于 scope，不依赖目录索引）。

验收：

1. 任意本地目录 `ms init` 后可看到稳定 project identity 和 manifest。
2. 文件移动但 contentHash 不变时 identity 不变（manifest 随 `.mengshu.json` 指针保留）。
3. `ms project context` 能输出当前 workspace/project 的 5 slot。
...
4. 完整 `ms project index/refresh/watch/add-root` + 多 source root + scanner 改造。
5. 第二 appId adapter（claw-research 或 claw-project）。
6. cross-product 黄金集（30 条）。

验收（v0.2）：

1. `ms project context --app-id openclaw` 与 `--app-id claw-research` 在同一 workspace 下复用 profile/rules。
...
4. `ms init -> ms project context` 能在 10 分钟内跑通（单 appId，不依赖目录索引）。
...
1. `eval/goldens/mengshu-v0.1.jsonl`（单 appId）。
2. `eval/goldens/mengshu-safety.jsonl`。
3. quick eval runner。
...
| Project Memory Workspace + `.mengshu.json` schema | `docs/03-architecture/product-positioning.md`、`docs/05-api/cli-commands.md` |
...
| `ms init/project/doctor/demo/connect/reset` | `docs/05-api/cli-commands.md` |
...
| 编号 | 风险 | 严重性 | 处理 |
|------|------|--------|------|
| RISK-1 | 本地数据库损坏（LanceDB 断电/磁盘满） | 高 | `ms doctor` 检查磁盘 + `ms backup`/`ms restore`；evidence append-only |
| RISK-2 | 跨 appId 复用的隐私边界（产品 A 偏好被 B 用，是否符合预期） | 高 | 复用默认 opt-in，`ms init`/Console 可关闭；只默认共享 profile/rules；ADR 记录策略 |
...
| RISK-8 | `ms watch` 在 inotify limit / Spotlight 下表现 | 中 | doctor 检查 inotify limit，超限提示用户 |
...
3. **Milestone A2-lite**：`ms init` 创建 project scope identity 和 manifest（不强制目录索引）。
4. **Milestone B**：`ms doctor/demo/connect/status`（单 appId 验证）。
5. **Milestone C**：Console Candidates + candidate 审核闭环。
6. **Milestone D-lite**：v0.1 + safety quick eval + release gate。

v0.1 完成后，mengshu 在**单 appId 内**实现可验证的 Working Context 闭环：5 slot 低延迟注入、注入安全可解释、scope 隔离正确、记忆可治理。
