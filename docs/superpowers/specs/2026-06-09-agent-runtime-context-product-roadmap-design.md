# mengshu Agent Runtime Context 产品方案与路线图

> 日期：2026-06-09
> 状态：设计规格，等待评审
> 读者：mengshu 内部研发与产品团队
...
> - `docs/03-architecture/mengshu-deep-optimization-architecture.md`
...
| D2 | mengshu 的核心价值是让 Agent 越用越懂用户、越理解用户的工作 | 路线图必须覆盖用户偏好、工作模式、项目背景、历史经验的持续沉淀 |
...
| D10 | 本地目录可以被初始化为 Project Memory Workspace | 目录是用户工作上下文的 project root，`ms init` 建立 identity、manifest、scope 和后续增量更新链路 |

---

## 2. 产品主线

mengshu 是面向 Agent 应用的本地优先记忆中间件，核心服务对象是 **用户持续存在的工作上下文**。
...
> mengshu 提供面向用户工作上下文的 Agent Runtime Context Layer，用 5 type / 5 slot 将长期记忆整理成可注入、可查找、可追溯、可治理、可评测的任务上下文。
...
存储视图回答的问题是：mengshu 到底保存什么，如何保证长期记忆可找、可追溯、可撤销、可重放。

5 type 是运行视图；存储视图不能直接等同于 5 type。结合 Mem0、Zep/Graphiti、Cognee、Letta、LangMem 和 LightRAG 的取舍，mengshu 的存储视图应遵循五个原则：
...
| Project workspace lifecycle | `ms init` / `ms project refresh` | 本地 project identity、source roots、目录层级和 ingest policy | Project manifest + Source/Evidence + index/tree delta jobs |
...
这意味着 `ms init` 不是把所有文件一次性塞进向量库，而是建立长期可增量维护的工作上下文边界：

1. **project identity**：为当前目录生成稳定 `projectId/workspaceId`，并记录用户、scope、创建时间和 schema version。
2. **project manifest**：在项目目录写入轻量指针文件，例如 `.mengshu.json`。
...
6. **incremental update chain**：后续通过 `ms project refresh`、`ms project watch` 或 `memory_session_commit` 局部更新，而不是每次全量重建。

推荐布局：

```text
/path/to/project/
  .mengshu.json       # 轻量指针和本项目声明，可选择是否纳入版本管理
...
2. `.mengshu.json` 只保存 project pointer、schema、local store id 和可审查配置；完整数据在用户本机全局库。
...
```text
ms project refresh / watch event
...
| 命令 | 作用 |
|------|------|
| `ms init` | 在当前目录初始化 Project Memory Workspace，生成本地 project identity 和 manifest |
| `ms project status` | 查看 project scope、source roots、索引新鲜度、候选区和失败 job |
| `ms project add-root <path>` | 增加一个 source root，并设置 role/include/exclude |
| `ms project index` | 首次索引当前 project workspace |
| `ms project refresh` | 对 source roots 做 manifest diff 和增量更新 |
| `ms project watch` | 监听文件变化并批量触发 refresh job |
| `ms project commit` | 把当前 session summary、决策、资源变化写回 project memory |
| `ms project lookup <query>` | 在当前 project scope 下速查记忆、资源和 evidence |
| `ms project context` | 预览当前 Agent Runtime 会拿到的 5 slot context |

`memory_session_commit` 和 `ms project refresh` 的边界：

| 动作 | 更新什么 | 不负责什么 |
|------|----------|------------|
| `memory_session_commit` | 会话摘要、用户决策、任务状态、运行经验、显式资源线索 | 文件系统全量扫描 |
| `ms project refresh` | 本地目录内容、文件新增/修改/删除、目录层级和文档证据 | 判断运行结论是否应成为长期规则 |
...
目标：把 mengshu 从“记忆存储模块”收敛成稳定的 Agent Runtime Context API。
...
1. `ms doctor`：检查 server、DB、embedding、scope、REST、Console、静态资源。
2. `ms demo`：演示同一用户工作上下文在两个 appId 间复用。
3. `ms connect openclaw`：输出 server URL、secret、scope 示例和 adapter 配置。
...
1. `eval/goldens/mengshu-v0.1.jsonl`
2. `eval/goldens/mengshu-cross-product.jsonl`
3. `eval/goldens/mengshu-safety.jsonl`
4. quick eval runner。
...
| 类型 | 套件 | 目的 |
|------|------|------|
| 内置黄金集 | `mengshu-v0.1` | 验证 context、lookup、slot、fallback |
| 内置黄金集 | `mengshu-cross-product` | 验证用户工作上下文跨接入方持续存在 |
| 内置黄金集 | `mengshu-safety` | 验证 private/revoked/stale/conflict 不误注入 |
...
4. 之后再推进 Console candidates 和 `ms doctor/demo/connect`。
