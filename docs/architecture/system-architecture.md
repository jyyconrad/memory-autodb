...
OpenClaw Plugin
  index.ts
    ...
    ├─ adapters/rest/ + server/  # REST router、Node HTTP daemon、agent-fast-path
    ├─ adapters/mcp/             # MCP Server（stdio/transport-agnostic facade）
    ├─ adapters/sdk/             # JS client（sdk/js 为兼容 re-export）
...
| JS SDK | ✅ v1.0.2 | 面向 REST API 的 client（`adapters/sdk/client.ts`，`sdk/js/client.ts` 兼容旧路径） |
...
ms project ingest-history --dry-run
  ...
  -> redaction.ts（敏感信息过滤）
  -> adapters/sources/{codex,claude-code,openclaw}
  -> dry-run 报告（不写库）
```
...
| CLI（`ms` 命令组） | `index.ts`、`adapters/openclaw/cli-*.ts` | ✅ 可用（init/doctor/why/recall/forget/project ingest-history/stats/search/scan/serve/mcp） |
| REST API | `adapters/rest/router.ts`、`server/daemon.ts` | ✅ 可用（/v1/health、/v1/memories、/v1/recall、/v1/context） |
| MCP Server | `adapters/mcp/server.ts`、`adapters/mcp/stdio-server.ts`、`adapters/mcp/tools.ts` | ✅ stdio 可用（8 个核心工具） |
| JS SDK | `adapters/sdk/client.ts` | ✅ REST client baseline（`sdk/js/client.ts` 兼容旧路径） |
