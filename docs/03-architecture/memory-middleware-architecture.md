...
> 日期：2026-05-30  
> 状态：设计方案  
> 目标：将 mengshu 从 OpenClaw 插件扩展为服务多产品 Agent Runtime 的记忆中间件系统
...
mengshu 当前已经具备长期记忆插件的核心能力：OpenClaw 工具注册、自动捕获/召回、目录扫描、LanceDB/Supabase/Hybrid 存储、多表 `memories` / `knowledge` 隔离，以及基础元数据丰富。但它的系统边界仍然是“OpenClaw 插件”，主要入口集中在 `index.ts`，其他产品难以稳定复用。

调研 `rohitg00/agentmemory` 与 `tinyhumansai/openhuman` 后，新的方向不应只是继续给插件增加表、命令或数据库 Provider，而是把 mengshu 抽象成一个独立的记忆中间件：
...
agentmemory 的定位更偏开发工具 agent 共享记忆服务，不是 mengshu 当前主战场。它的关键设计点仍有参考价值：

| 维度 | 设计 | 对 mengshu 的启发 |
|------|------|--------------------------|
| 系统形态 | 常驻 server，默认 REST 端口，另有 MCP/Hook/Viewer | mengshu 需要 `embedded` 和 `server` 两种运行模式 |
...
| 维度 | 设计 | 对 mengshu 的启发 |
|------|------|--------------------------|
| 本地优先 | SQLite + 本机 Markdown vault | mengshu 应保留本地可控、可导出的形态 |
...
| 外部后端 | 可选代理到外部 memory provider | mengshu 也应定义 backend/provider contract，允许外部记忆后端 |
...
| `remote` | 只运行 client adapter，连接远程 mengshu server | 团队共享、云端部署 |
| `backend-proxy` | mengshu 代理到 Mem0/Cognee/Graphiti 等外部 provider | 兼容已有记忆系统 |
...
3. 提供 `ms migrate --to-schema v4` 将旧记录回填到新模型。
4. 两个小版本后再考虑停止双写。
...
目标：让其他产品可以调用 mengshu。

- 增加本地 daemon：`ms serve` 或 `mengshu serve`。
...
完成后，mengshu 就从“OpenClaw 插件”变成“OpenClaw 可嵌入使用的记忆核心 + 可被其他产品调用的本机服务”。后续再按 Phase 2-5 扩展 ingestion、混合检索、图谱和摘要树。
...
- 当前 mengshu 文档与代码：`index.ts`、`config.ts`、`db/types.ts`、`docs/03-architecture/system-architecture.md`、`docs/04-design/04.1-overview/memory-plugin-design.md`
