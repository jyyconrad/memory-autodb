...
本目录记录 mengshu 的产品定位、系统架构、长期方案和架构评审。阅读时先区分当前实现、方案设计和外部参考。
...
| [global-config-directory-upgrade.md](./global-config-directory-upgrade.md) | v0.1.2 核心交付方案 | 从 `~/.openclaw/` 迁移到独立 `~/.mengshu/` 的配置、密钥、项目 registry 和兼容策略（1.0.2 分支主要更新） |
...
| [mengshu-deep-optimization-architecture.md](./mengshu-deep-optimization-architecture.md) | 长期优化方案 | 5 问题、5 type、SlotSnapshot、候选治理、图谱和记忆树路线 |
...
| 路径 | 状态 | 使用方式 |
|------|------|----------|
| [copy-from-mate/](./copy-from-mate/) | 外部参考 | Banto/iFlyMate 记忆系统设计输入，不是 mengshu 当前实现承诺 |
...
- 未来路线或评审结论优先更新 `mengshu-deep-optimization-architecture.md` 和对应评审文档。
