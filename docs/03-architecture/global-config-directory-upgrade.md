...
> 状态：升级设计稿  
> 适用范围：mengshu 从 OpenClaw 插件路径演进为独立本地优先记忆中间件后的全局目录、配置、密钥和项目 registry 约定。  
> 当前代码仍存在 `~/.openclaw/` 兼容路径；本文定义下一阶段目标路径和迁移规则。

## 1. 背景

mengshu 的产品定位已经从“OpenClaw 的长期记忆插件”升级为“面向 Agent 应用的本地优先记忆中间件”。因此全局文件不应继续挂在 `~/.openclaw/` 下面，否则会带来三个问题：
...
### 3.1 `~/.mengshu/config.json`

`config.json` 是 mengshu 的全局运行配置。它应该可提交到本机备份，但不应包含密钥明文。
...
2. 所有 `${...}` 占位符从 `~/.mengshu/.env` 解析。
3. `dbPath` 默认指向 `~/.mengshu/memory/lancedb`。
4. `projects.rootDir` 默认指向 `~/.mengshu/projects`。
...
### 4.2 项目指针 `.mengshu.json`

项目目录里的 `.mengshu.json` 只保存足够找到全局项目目录的信息：
...
```json
{
  ...
  "projectId": "proj-xxxx",
  "workspaceId": "ws-xxxx",
  "globalManifest": "~/.mengshu/projects/proj-xxxx/manifest.json",
...
2. 目录移动时，只要 `.mengshu.json` 随目录保留，就继续复用原长期记忆。
...
```json
{
  ...
  "displayName": "mengshu",
  "rootHints": [
    "/Users/user/develop/code/work_code/openclaw_plugins/mengshu"
  ],
  "scopeDefaults": {
    "tenantId": "local",
    "userId": "default",
  ...
  "sourceRoots": [
    {
      "id": "src-project-root",
      "path": "/Users/user/develop/code/work_code/openclaw_plugins/mengshu",
      "role": "project_root",
...
command = "/path/to/mengshu/node_modules/.bin/tsx"
args = ["/path/to/mengshu/scripts/mengshu-mcp.ts"]

[mcp_servers.memory_autodb.env]
MEMORY_AUTODB_CONFIG = "/Users/<user>/.mengshu/config.json"
MEMORY_AUTODB_ENV = "/Users/<user>/.mengshu/.env"
NO_PROXY = "127.0.0.1,localhost,::1"
no_proxy = "127.0.0.1,localhost,::1"
```
...
OpenClaw 仍可通过插件 slot 接入，但插件配置应引用独立目录，而不是把 mengshu 状态放在 `~/.openclaw/`：
...
```json
{
  ...
  "entries": {
    "mengshu": {
      "enabled": true,
      "configPath": "~/.mengshu/config.json",
      "envPath": "~/.mengshu/.env"
    }
  }
}
...
```bash
mkdir -p ~/.mengshu/{memory/lancedb,projects,logs,backups}
chmod 700 ~/.mengshu
touch ~/.mengshu/.env
chmod 600 ~/.mengshu/.env
```
...
~/.mengshu/config.json
~/.mengshu/.env
~/.mengshu/memory/lancedb
~/.mengshu/projects/<projectId>/
```

建议迁移步骤：

1. 创建 `~/.mengshu/` 目录并设置权限。
2. 将旧 `~/.openclaw/mengshu-mcp.json` 转换为 `~/.mengshu/config.json`。
3. 将 mengshu 相关密钥从 `~/.openclaw/.env` 拆到 `~/.mengshu/.env`。
4. 将 `~/.openclaw/memory/lancedb` 迁移或复制到 `~/.mengshu/memory/lancedb`。
...
2. 写入项目指针 `/path/to/project/.mengshu.json`。
3. 创建全局项目目录 `~/.mengshu/projects/<projectId>/`。
4. 写入完整 `manifest.json`。
5. 更新 `~/.mengshu/registry.json`。

## 7. Registry 设计

`~/.mengshu/registry.json` 是本机项目快速索引，不是长期唯一真源；长期真源仍是每个项目的 `manifest.json`。
...
```json
{
  ...
  "projects": {
    "proj-xxxx": {
      "workspaceId": "ws-xxxx",
      "displayName": "mengshu",
      "manifestPath": "~/.mengshu/projects/proj-xxxx/manifest.json",
      "lastSeenRoot": "/Users/user/develop/code/work_code/openclaw_plugins/mengshu",
...
2. 否则读取 `~/.mengshu/config.json`。
3. 若不存在，再兼容读取 `~/.openclaw/mengshu-mcp.json` 或 `~/.openclaw/conf/plugins.json`。
4. 读取旧路径时应输出 warning，提示用户迁移。

项目 manifest 兼容：

1. v0.1 `.mengshu.json` 直接包含较多字段；v0.2 指针化后应支持自动升级。
2. 如果项目目录只有旧 `.mengshu.json`，`ms init` 不应覆盖 identity，而应创建全局项目目录并回填指针。
3. 如果 `~/.mengshu/projects/<projectId>/manifest.json` 缺失，应允许从项目指针做恢复，但要标记为 degraded。

## 9. 验收标准

升级完成后应满足：

1. `~/.mengshu/config.json` 和 `~/.mengshu/.env` 是全局配置和密钥真源。
2. Codex、Claude、OpenClaw 都指向同一套 mengshu 全局配置。
3. 每个 Project Memory Workspace 都有 `~/.mengshu/projects/<projectId>/manifest.json`。
4. 项目目录只保存轻量 `.mengshu.json` 指针。
5. `memory_health` 可用。
...
1. 更新 `scripts/mengshu-mcp.ts` 默认路径：`~/.mengshu/config.json` 与 `~/.mengshu/.env`。
2. 扩展配置解析：支持 `configPath/envPath`，保留旧 `config` 内联模式。
3. 修改 `ms init`：同时写项目指针和全局 `projects/<projectId>/manifest.json`。
4. 新增 `registry.json` 读写模块。
5. 新增迁移命令：`ms migrate-home --from ~/.openclaw --to ~/.mengshu --dry-run`。
