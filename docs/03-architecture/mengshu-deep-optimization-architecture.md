...
> 日期：2026-06-09  
> 状态：架构方案（v2 评审修订）  
> 范围：在现有 v4 记忆中间件基线之上，吸收 `copy-from-mate/memory-system-upgrade-design.md` 的 5 问题、5 type、记忆树、图谱、升格治理思想，形成 mengshu 的长期优化方案。  
> 关联文档：  
...
mengshu 的下一阶段不应只是“OpenClaw 的长期记忆插件”，也不应只是“向量库 + REST API”。它的产品定位是 **多产品 Agent Runtime 的本地优先记忆方案**：当同一个用户切换不同 OpenClaw 类型产品时，工作记忆、协作偏好、长期约束、历史经验和可用资源仍然持续存在。但 v0.x 的默认落地范围必须收敛：**先以 LanceDB + 本地文件 + embedding API 的单机配置，做成一个快、准、可回放的本地记忆中间件**；远程同步、开放平台和复杂治理放到真实需求出现后的 v1+。

它的长期目标是服务多个 OpenClaw 类产品和 Agent Runtime 的工作记忆中间件：
...
v2 修订后的关键边界：5 问题/5 type 是 **Agent Runtime 上下文视图**，不是 v0.x 主库的硬约束。主库保留通用 `kind` 与原始 evidence；能归入 5 type 的节点参与 5 槽位上下文，无法稳定归类的节点仍可通过 `memory_lookup` 检索。这样既保留产品语义，又避免在研究、客服、内容生产、项目协作等真实场景中把有价值事实强行丢弃。
...
mengshu 不负责保存所有对话、工具调用、连接器原始响应或执行日志。它只保存对未来行为有稳定影响的信息。
...
mengshu 的能力成长应从“可被证明更可靠”出发，而不是不断增加新工具、新表和新图谱。每个新能力必须回答四个问题：
...
mengshu 的长期成长建议按 6 个成熟度层级推进。每一级都必须保持上一层稳定，而不是用更智能的能力替代基础能力。
...
长期来看，mengshu 的能力提升不是靠单次大模型调用，而是靠闭环飞轮：
...
```text
┌──────────────────────────────────────────────────────────────────────┐
│ Product / Agent Runtime Layer                                         │
│ OpenClaw App A | OpenClaw App B | OpenHuman | Banto | Custom Apps     │
└───────────────────────────────┬──────────────────────────────────────┘
                                │
┌───────────────────────────────▼──────────────────────────────────────┐
│ Adapter / Protocol Layer                                              │
│ OpenClaw Adapter | REST | MCP | JS SDK | CLI | Runtime Hook Adapter   │
└───────────────────────────────┬──────────────────────────────────────┘
                                │
┌───────────────────────────────▼──────────────────────────────────────┐
│ Memory Gateway                                                        │
...
│ Audit | Vault Export          │ │ Graph | Tree     │ │ provider bridge │
└───────────────────────────────┘ └──────────────────┘ └────────────────┘
```
...
`slot` tree 是本方案相对 OpenHuman 的新增适配：OpenHuman 的 source/topic/global tree 解决来源、主题、时间；mengshu 还需要把 5 个本质问题稳定组织成 prompt slot，因此需要 slot tree 或 slot snapshot。
...
mengshu 需要同时支持实体关系和记忆演化关系，但底层不应拆成两套 repository。v0.x 应使用统一 `GraphRepository`，通过 `nodeType` 和 `edgeType` 区分：
...
| project | 工作项目、业务项目 | task_context、experience |
...
| local-server | 本机 daemon，多个本地 OpenClaw 类产品共享 | OpenClaw App A / App B 共用 |
| remote-client | v1+，本机只有 adapter，连接远程服务 | 团队部署 |
| remote-server | v1+，远程 mengshu 服务 | 企业共享 |
| backend-proxy | v1+，代理到 Mem0/Cognee/Graphiti 等 provider | 对接外部后端 |
...
mengshu 是 Agent 的上下文基础设施，因此可靠性目标不是传统“服务可用”就够了，还包括“不能给 Agent 错误或越权的上下文”。
...
mengshu 作为中间件，长期开放性有四个层次：

1. **多 Runtime 接入**：OpenClaw 类型产品、OpenHuman、Banto、自研 Agent Runtime 都能通过 REST/MCP/SDK 使用。
2. **多产品嵌入**：产品可以只用 Agent Runtime 快路径，也可以使用 Console、治理和同步能力。
...
4. `ms migrate --to-schema v5 --dry-run` 输出迁移计划，显示每条旧记录的 `kind`、可选 `semanticType` 或 fallback 原因。
...
目标：让 OpenClaw 类型产品中的 Agent Runtime 通过一次调用获得 5 段上下文答案。
...
5. 不把 Mem0、Cognee、Graphiti 等外部 provider 作为核心依赖前提。
...
这些边界能保护 mengshu 从“什么都存”退化成“不可控的信息堆”。
...
| v0.x 开发计划 | `docs/04-design/04.2-detail/mengshu-v0x-development-plan.md` | 5 个 milestone、测试、迁移 |
