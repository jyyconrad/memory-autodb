...
memory-autodb 的下一阶段不应只是“OpenClaw 的长期记忆插件”，也不应只是“向量库 + REST API”。它的产品定位是 **多产品 Agent Runtime 的本地优先记忆方案**：当同一个用户切换不同 OpenClaw 类型产品时，工作记忆、协作偏好、长期约束、历史经验和可用资源仍然持续存在。但 v0.x 的默认落地范围必须收敛：**先以 LanceDB + 本地文件 + embedding API 的单机配置，做成一个快、准、可回放的本地记忆中间件**；远程同步、开放平台和复杂治理放到真实需求出现后的 v1+。
